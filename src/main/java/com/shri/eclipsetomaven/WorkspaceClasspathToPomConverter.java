...
public class WorkspaceClasspathToPomConverter {
    ...
    private void printDependencies(Document pomXmlDoc) {
		if("true".equals(ApplicationConfig.INSTANCE.getValue(PRINT_DEPENDENCY_GRAPH))){
			Element documentRoot = pomXmlDoc.getDocumentElement();
			String projectName = XMLUtil.getTagValue(documentRoot, NAME);
			print(projectName);
			print("=========================");
			List<Element> dependencies = XMLUtil.getElements(DEPENDENCY, documentRoot);
			for (Element dependency : dependencies) {
				String dependencyName = XMLUtil.getTagValue(dependency, ARTIFACT_ID);
				String versionNumber = XMLUtil.getTagValue(documentRoot, VERSION);
	...
	private void print(String str) {
		String printIoType = ApplicationConfig.INSTANCE.getValue(PRINT_DEPENDENCY_GRAPH_IOTYPE);
		if(PRINT_DEPENDENCY_GRAPH_IOTYPE_CONSOLE.equals(printIoType)){
			...
			String fileName = ApplicationConfig.INSTANCE.getValue(PRINT_DEPENDENCY_GRAPH_FILEPATH);
